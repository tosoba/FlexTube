...
class MainRepository @Inject constructor(
    ...
    private fun handlePlaylistItems(
        ...
        Single.error { IllegalStateException(errorMessage) }
    }
    ...
    override fun getGeneralHomeItems(
            ...
            .flatMap { (savedVideos, nextPageToken) ->
                handlePlaylistItems(
                        ...
                        savedVideos,
                        nextPageToken,
                        retrieveAndSaveGeneralHomeItems(accessToken, nextPageToken),
                        "No more home items to retrieve."
                )
            }
    ...
    override fun getHomeItemsByCategory(
            ...
            .flatMap { (savedVideos, nextPageToken) ->
                handlePlaylistItems(
                        ...
                        savedVideos,
                        nextPageToken,
                        retrieveAndSaveHomeItemsFromCategory(categoryId, nextPageToken),
                        "No more home items to retrieve."
                )
            }
            .map { (videos, _) -> videos.map(PlaylistItemMapper::toDomain) }
    ...
    override fun loadRelatedVideos(
            ...
            .flatMap { (savedVideos, nextPageToken) ->
                handlePlaylistItems(
                        ...
                        savedVideos,
                        nextPageToken,
                        retrieveAndSaveRelatedVideos(videoId, nextPageToken),
                        "No more related videos to retrieve."
                )
            }
            .map { (videos, _) -> videos.map(PlaylistItemMapper::toDomain) }
    ...
    override fun searchForVideos(
            query: String,
            shouldReturnAll: Boolean
    ): Single<List<PlaylistItem>> = youtubeCachedDataStore.getSavedFoundVideos(query)
            .flatMap { (savedVideos, nextPageToken) ->
                handlePlaylistItems(
                        shouldReturnAll,
                        savedVideos,
                        nextPageToken,
                        retrieveAndSaveSearchedVideos(query, nextPageToken),
                        "No more related videos to retrieve."
                )
            }
            .map { (videos, _) -> videos.map(PlaylistItemMapper::toDomain) }

    private fun retrieveAndSaveSearchedVideos(
            query: String,
            pageToken: String?
    ): Single<Pair<List<PlaylistItemData>, String?>> = youtubeRemoteDataStore.searchForVideos(query, pageToken)
            .doOnSuccess { (videos, nextPageToken) ->
                youtubeCachedDataStore.saveFoundVideos(query, videos, nextPageToken)
            }
    ...
    private fun getPlaylistData(
            channelIds: List<String>
    ): Single<List<PlaylistData>> = Single.zip(channelIds.map { youtubeCachedDataStore.getPlaylistByChannelId(it) }) { playlists ->
        playlists.map { it as PlaylistData }
    }

    @Suppress("UNCHECKED_CAST")
    override fun getSavedVideosWithUpdates(
            channelIds: List<String>
    ): Flowable<List<PlaylistItem>> = Flowable.zip(channelIds.map { youtubeCachedDataStore.getSavedVideosFlowable(it) }) { savedVideos ->
        (savedVideos.map { it as List<PlaylistItemData> }).toList().flatten()
    }.map {
    ...
    override fun getSavedVideos(
            channelIds: List<String>
    ): Single<List<PlaylistItem>> = Single.zip(channelIds.map { youtubeCachedDataStore.getSavedVideos(it) }) { savedVideos ->
        (savedVideos.map { it as List<PlaylistItemData> }).toList().flatten()
    }.map {
        it.map(PlaylistItemMapper::toDomain)
    }
